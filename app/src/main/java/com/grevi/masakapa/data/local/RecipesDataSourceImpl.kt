...
class RecipesDataSourceImpl @Inject constructor(private val recipesDAO: RecipesDAO) : RecipesDataSource {
    ...
    override suspend fun getFlowRecipes(): Flow<MutableList<RecipesTable>> = flow { emit(recipesDAO.getAllMarkRecipes()) }
    ...
    override suspend fun insertDetail(detailTable: DetailTable) = recipesDAO.insertDetail(detailTable)
    override suspend fun findDetail(name: String): Flow<List<DetailWithIngredientsAndSteps>> = flow { emit(recipesDAO.findDetail(name)) }
    override suspend fun insertFavorite(favorite: RecipeFavorite) = recipesDAO.insertFavorite(favorite)
